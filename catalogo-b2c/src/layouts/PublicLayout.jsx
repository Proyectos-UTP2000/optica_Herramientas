...
const PublicLayout = () => {
  ...
  const styles = {
    ...
    userButton: { display: "flex", alignItems: "center", gap: "6px", border: "1px solid #cbd5e1", borderRadius: "20px", padding: "6px 12px", backgroundColor: "#fff", cursor: "pointer", fontSize: "14px", fontWeight: "600", color: "#334155" },
    dropdown: { position: "absolute", top: "100%", right: 0, marginTop: "8px", backgroundColor: "#fff", border: "1px solid #e2e8f0", borderRadius: "8px", boxShadow: "0 10px 15px -3px rgba(0,0,0,0.1)", display: menuOpen ? "block" : "none", minWidth: "160px", zIndex: 200 },
    dropdownLink: { display: "block", width: "100%", textAlign: "left", padding: "10px 16px", border: "none", background: "none", color: "#334155", textDecoration: "none", fontSize: "14px", cursor: "pointer", transition: "background-color 100ms" },
    authLink: { fontSize: "14px", fontWeight: "600", color: "#2563eb", textDecoration: "none", cursor: "pointer" }
  };
        ...
        <div style={styles.navbarActions}>
          ...
          {usuario ? (
            <div style={styles.userMenu}>
              <button style={styles.userButton} onClick={() => setMenuOpen(!menuOpen)}>
                <PersonCircle size={18} style={{ color: "#2563eb" }} />
                <span>{usuario.split("@")[0]}</span>
              </button>
              <div style={styles.dropdown} onMouseLeave={() => setMenuOpen(false)}>
                <Link to="/mi-cuenta" style={styles.dropdownLink} onClick={() => setMenuOpen(false)}>
                  Mi Perfil / Pedidos
                </Link>
            ...
            <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
              <button 
                ...
                style={{ ...styles.dropdownLink, color: "#2563eb", fontWeight: "600", padding: "6px 12px" }}
              >
                Iniciar Sesión
              </button>
            </div>

...
const Ventas = () => {
  ...
  const handleImportarCotizacionDirecta = async (
    quoteId,
    listClientes,
    listProductos,
  ) => {
    ...
    try {
      ...
      if (coti.estado && coti.estado.toUpperCase() === "PROCESADO") {
        Toast.fire({
          icon: "warning",
          title: "Esta cotización ya fue procesada",
        });
        return;
      }
      ...
      let clienteEncontrado = listClientes.find(
        (c) => c.numeroDocumento === coti.clienteDocumento,
      );
      ...
      for (const det of coti.detalles) {
        const prod = listProductos.find((p) => p.id === det.productoId);
        if (!prod) {
          advertenciasStock.push(
            `Producto #${det.productoId} (${det.productoNombre}) no existe en el catálogo.`,
          );
          continue;
        }
        ...
        if (stockDisponible <= 0) {
          advertenciasStock.push(
            `Producto "${prod.nombre}" no cuenta con stock disponible.`,
          );
          continue;
        }
        ...
        if (cantidadAAgregar < det.cantidad) {
          advertenciasStock.push(
            `Producto "${prod.nombre}" solo tiene ${stockDisponible} unidades disponibles.`,
          );
        }
      ...
      if (nuevosDetalles.length > 0) {
        ...
        Toast.fire({
          icon: "error",
          title:
            "No se pudo agregar ningún producto al carrito (sin stock o descontinuados).",
        });
      }
    } catch (err) {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      const activeClientes = (clientesRes.data || []).filter(
        (cliente) => cliente.estado === 1,
      );
      const activeProductos = (productosRes.data || []).filter(
      ...
      if (cotizacionIdParam) {
        await handleImportarCotizacionDirecta(
          cotizacionIdParam,
          activeClientes,
          activeProductos,
        );
  ...
  const handleImportarCotizacion = async () => {
    ...
    try {
      ...
      if (!clienteEncontrado && coti.clienteDocumento) {
        ...
        const listClientes = (clientesRes.data || []).filter(
          (c) => c.estado === 1,
        );
        setClientes(listClientes);
        clienteEncontrado = listClientes.find(
          (c) => c.numeroDocumento === coti.clienteDocumento,
        );
      }
  ...
  const enviarVenta = async (event) => {
    ...
    try {
      await api.post("/api/v1/ventas", {
        ...
        cotizacionId: cotizacionImportada
          ? Number(cotizacionImportada.id)
          : null,
          ...
          >
            ...
            >
              ...
              {cotizacionImportada && (
                ...
                >
                  Importada: #{cotizacionImportada.id} (
                  {formatoMoneda(cotizacionImportada.totalEstimado)})
                </span>
              )}
            </div>

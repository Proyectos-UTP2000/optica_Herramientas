  ...
  return (
    <div style={{ padding: "10px 0" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, gap: 12 }}>
        ...
        <button className="btn-secondary" onClick={cargarReporte} disabled={cargando}>
          <ArrowRepeat className={cargando ? "spin-animation" : ""} /> Actualizar
        </button>
      </div>

      {cargando && (
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 8, padding: 32, color: "#64748b", textAlign: "center" }}>
          Cargando reporte...
        </div>
      )}
      ...
      {!cargando && sinCaja && (
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 8, padding: 32, color: "#64748b", textAlign: "center" }}>
          <CashCoin style={{ fontSize: 28, color: "#94a3b8", marginBottom: 8 }} />
          <h3 style={{ margin: "0 0 6px", color: "#0f172a" }}>No hay caja abierta</h3>
          <p style={{ margin: 0 }}>Abre una caja desde el boton superior para generar el reporte diario.</p>
        </div>
      )}
      ...
      {!cargando && reporte && (
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "minmax(260px, 0.45fr) minmax(420px, 1fr)", gap: 16 }}>
            <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 8, padding: 18 }}>
              <h3 style={{ margin: "0 0 12px", fontSize: 16, color: "#0f172a" }}>Totales por medio</h3>
              {totalesPorMedio.length === 0 ? (
                <p style={{ margin: 0, color: "#94a3b8" }}>Sin ventas registradas.</p>
              ) : (
                <div style={{ display: "grid", gap: 8 }}>
                  {totalesPorMedio.map(([medio, total]) => (
                    <div key={medio} style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: "10px 0", borderBottom: "1px solid #f1f5f9" }}>
                      <span style={{ color: "#475569", fontWeight: 600 }}>{medio}</span>
                      <strong style={{ color: "#166534" }}>{formatoMoneda(total)}</strong>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 8, padding: 18 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12, gap: 12 }}>
                <h3 style={{ margin: 0, fontSize: 16, color: "#0f172a" }}>Ventas del dia</h3>
                <span style={{ color: "#64748b", fontSize: 13 }}>{reporte.cantidadVentas || 0} venta(s)</span>
              </div>
                ...
                <table className="table-custom" style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                  <thead>
                    <tr style={{ borderBottom: "2px solid #e2e8f0", color: "#475569" }}>
                      ...
                      <th style={{ textAlign: "left", padding: 10 }}>Comprobante</th>
                  ...
                  <tbody>
                    {(reporte.ventas || []).length === 0 ? (
                      <tr><td colSpan="5" style={{ padding: 28, textAlign: "center", color: "#94a3b8" }}>Sin ventas en esta caja.</td></tr>
                    ) : reporte.ventas.map((venta) => (
                      <tr key={venta.ventaId} style={{ borderBottom: "1px solid #f1f5f9" }}>
                        <td style={{ padding: 10 }}>{formatoHora(venta.fecha)}</td>
                        <td style={{ padding: 10, fontWeight: 600 }}>{venta.clienteNombre || "---"}</td>
                        <td style={{ padding: 10 }}>{venta.numeroComprobante || "---"}</td>

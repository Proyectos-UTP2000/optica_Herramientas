...
const ModalCrearMarca = ({ cerrarModal, recargarTabla }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.post("/api/v1/marcas", { nombre: nombre.trim() }, { headers });
            Toast.fire({ icon: "success", title: "Marca creada correctamente" });
            recargarTabla();
            cerrarModal();
        } catch (e) {
            mostrarAlerta("Error al crear", e.response?.data?.message || "No se pudo crear la marca.", "error");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div style={S.overlay} onClick={(e) => e.target === e.currentTarget && cerrarModal()}>
            <div style={S.modal}>
                <div style={S.header}>
                    <h3 style={S.titulo}>Nueva Marca</h3>

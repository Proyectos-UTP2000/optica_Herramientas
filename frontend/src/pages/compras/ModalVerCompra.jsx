...
export default function ModalVerCompra({ compra, onCerrar, onActionCompleted }) {
    ...
    const claseMedioPago = (mp) => {
        const map = {
            EFECTIVO: 'badge--efectivo', TRANSFERENCIA: 'badge--transferencia',
            TARJETA: 'badge--tarjeta', YAPE: 'badge--yape',
            PLIN: 'badge--plin', OTRO: 'badge--otro',
        };
            ...
            <div className="modal-content" style={{ width: '820px' }} onClick={e => e.stopPropagation()}>
                ...
                <div className="modal-body modal-body--flush">
                    ...
                    <div className="info-card-grid">
                        <InfoCard label="Proveedor" icon={<PersonIcon />} value={compra.proveedorNombre ?? '—'} />
                        <InfoCard label="Empleado" icon={<UserIcon />} value={compra.empleadoNombre ?? '—'} />
                        <InfoCard
                            label="Comprobante"
                            icon={<HashIcon />}
                            value={
                                <span className="td-code">
                                    {compra.compraNumeroCombrobante ?? compra.numeroComprobante ?? '—'}
                                </span>
                            }
                        />
                        <InfoCard
                            label="Forma de pago"
                            icon={<CardIcon />}
                            value={<span className={`badge ${claseFormaPago(compra.formaPago)}`}>{compra.formaPago ?? '—'}</span>}
                        />
                        <InfoCard
                            label="Medio de pago"
                            icon={<WalletIcon />}
                            value={<span className={`badge ${claseMedioPago(compra.medioPago)}`}>{compra.medioPago ?? '—'}</span>}
                        />
                        {compra.formaPago === 'CREDITO' && (<>
                            <InfoCard label="Cuotas" icon={<CalIcon />} value={compra.cuotas ?? '—'} />
                            <InfoCard label="Vencimiento" icon={<CalIcon />} value={compra.fechaVencimiento ?? '—'} />
                            <InfoCard
                                label="Deuda pendiente"
                                icon={<AlertIcon />}
                                value={<span className="total-row__value--descuento" style={{ color: '#dc2626', fontWeight: 700 }}>S/ {Number(compra.deuda ?? 0).toFixed(2)}</span>}
                            />
                        </>)}
                    </div>
                    ...
                    {compra.notaRecepcion && (
                        <div className="nota-recepcion">
                            ...
                            <span><strong>Nota:</strong> {compra.notaRecepcion}</span>
                        </div>
                    )}

                    {/* ── TABLA PRODUCTOS ── */}
                    <div className="section-label-wrapper">
                        <p className="section-label">Productos comprados ({detalles.length})</p>
                    </div>
                                ...
                                {detalles.length === 0 ? (
                                    ...
                                    detalles.map((d, i) => (
                                        <tr key={i}>
                                            ...
                                            <td className="td-name">{d.productoNombre ?? `ID: ${d.productoId}`}</td>
                                            ...
                                            <td className="text-right">S/ {Number(d.costoUnitario ?? d.precio ?? 0).toFixed(2)}</td>
                                            <td className="text-right" style={{ fontWeight: 600 }}>S/ {Number(d.subtotal ?? 0).toFixed(2)}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
                    ...
                    <div className="totales-wrapper">
                        <div className="totales-box">
                            <div className="total-row">
                                ...
                                <span className="total-row__value">S/ {subtotal.toFixed(2)}</span>
                            </div>
                            {descuento > 0 && (
                                <div className="total-row">
                            ...
                            <div className="totales-divider">
                                ...
                                <span className="totales-divider__value">S/ {total.toFixed(2)}</span>
                            </div>
                        </div>
                    </div>
                </div>

...
const ModalGestionEtiquetas = ({ onClose }) => {
  ...
  const cargar = async () => {
    ...
    try {
      ...
      Toast.fire({ icon: "error", title: "No se pudieron cargar las etiquetas" });
    } finally {
      setCargando(false);
    }
  };
  ...
  const handleCrear = async () => {
    ...
    if (!nombre) return;
    if (nombre.length < 2) {
      Toast.fire({ icon: "warning", title: "El nombre debe tener al menos 2 caracteres" });
      return;
    }
    if (nombre.length > 50) {
      Toast.fire({ icon: "warning", title: "El nombre no debe superar los 50 caracteres" });
      return;
    }
    if (etiquetas.some(e => e.nombre.toLowerCase() === nombre.toLowerCase())) {
      Toast.fire({ icon: "warning", title: "Ya existe una etiqueta con ese nombre" });
      return;
    }
    setGuardando(true);
    try {
      await axios.post("/api/v1/etiquetas", { nombre }, {
        headers: { Authorization: `Bearer ${token()}` }
      });
      setNuevaNombre("");
      ...
      Toast.fire({ icon: "error", title: err.response?.data?.message || "Error al crear la etiqueta" });
    } finally {
      setGuardando(false);
    }
  };
  ...
  const handleToggleEstado = async (etiqueta) => {
    try {
      await axios.patch(`/api/v1/etiquetas/${etiqueta.id}/estado`, {}, {
        headers: { Authorization: `Bearer ${token()}` }
      });
      Toast.fire({ icon: "success", title: etiqueta.estado === 1 ? "Etiqueta desactivada" : "Etiqueta activada" });
      cargar();
    } catch {
  ...
  const handleGuardarEdicion = async (id) => {
    ...
    try {
      await axios.put(`/api/v1/etiquetas/${id}`, { nombre }, {
        headers: { Authorization: `Bearer ${token()}` }
      });
      setEditandoId(null);
      ...
      Toast.fire({ icon: "error", title: err.response?.data?.message || "Error al actualizar" });
    }
  };
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px" }}>
          ...
          <div style={{ marginBottom: "20px" }}>
            ...
            <span style={{ fontSize: "11px", color: "#94a3b8", marginTop: "4px", display: "block" }}>
              2–50 caracteres, nombre único. Presiona Enter o haz clic en Agregar.
            </span>
          </div>
                  ...
                  {editandoId === e.id ? (
                    <>
                      <input
                        ...
                        onKeyDown={(ev) => { if (ev.key === "Enter") handleGuardarEdicion(e.id); if (ev.key === "Escape") setEditandoId(null); }}
                        maxLength={50}
                    ...
                    <>
                      <span style={{
                        ...
                        {e.nombre}
                        {e.estado !== 1 && (
                          <span style={{ marginLeft: "8px", fontSize: "10px", color: "#ef4444", fontWeight: "400" }}>
                            (Inactiva)
                          </span>
                        )}
                      </span>
                      ...
                      >
                        {e.estado === 1 ? <Trash size={14} /> : <CheckCircle size={14} />}
                      </button>
                    </>
                  )}
                </div>
...
const CatalogoWeb = () => {
  ...
  const handleToggleVisibilidad = async (p) => {
    ...
    const productData = {
      nombre: p.nombre, codigo: p.codigo, modelo: p.modelo, descripcion: p.descripcion,
      precio: p.precio, costo: p.costo, stockMinimo: p.stockMinimo, tipoProducto: p.tipoProducto,
      idCategoria: p.idCategoria, idMarca: p.idMarca, idUnidadVenta: p.idUnidadVenta,
      idUnidadCompra: p.idUnidadCompra, factorConversion: p.factorConversion,
      visibleWeb: nuevoEstado, destacado: p.destacado, slug: p.slug,
      descripcionWeb: p.descripcionWeb, idEtiquetas: p.etiquetas ? p.etiquetas.map(e => e.id) : [],
      orden: p.orden
    };
    ...
    try {
      ...
      Toast.fire({ icon: "success", title: nuevoEstado ? "Producto visible en la web" : "Producto ocultado de la web" });
      cargarProductos();
    } catch (error) {
      console.error(error);
  ...
  const handleToggleDestacado = async (p) => {
    ...
    const productData = {
      ...
      idCategoria: p.idCategoria, idMarca: p.idMarca, idUnidadVenta: p.idUnidadVenta,
      idUnidadCompra: p.idUnidadCompra, factorConversion: p.factorConversion,
      visibleWeb: p.visibleWeb, destacado: nuevoDestacado, slug: p.slug,
      descripcionWeb: p.descripcionWeb, idEtiquetas: p.etiquetas ? p.etiquetas.map(e => e.id) : [],
      orden: p.orden
    };
    const formData = new FormData();
    ...
    try {
      ...
      Toast.fire({ icon: "success", title: nuevoDestacado ? "Producto marcado como Destacado" : "Producto desmarcado" });
      cargarProductos();
    } catch (error) {
      console.error(error);
      Toast.fire({ icon: "error", title: "No se pudo actualizar el destacado" });
      setProductos(prev => prev.map(item => item.id === p.id ? { ...item, destacado: !nuevoDestacado } : item));
    }
  };
    ...
    <div className="container-fluid" style={{ padding: "10px 0" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "16px", marginBottom: "20px" }}>
        ...
        <div style={{ 
          ...
          color: "#fff", padding: "16px", borderRadius: "12px", boxShadow: "0 4px 6px -1px rgb(0 0 0 / 0.1)" 
        }}>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: "12px 16px", textAlign: "center" }}>
                        ...
                        >
                          {p.visibleWeb ? <Eye size={20} /> : <EyeSlash size={20} />}
                        </button>
                      </td>
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", gap: "4px", flexWrap: "wrap" }}>
                          {p.etiquetas && p.etiquetas.length > 0 ? (
                            p.etiquetas.map((t) => (
                              <span key={t.id} style={{ fontSize: "10px", background: "#eff6ff", color: "#2563eb", padding: "1px 6px", borderRadius: "4px", fontWeight: "600" }}>
                                {t.nombre}
                              </span>
                            ))
                          ) : (
      ...
      {modalEtiquetasAbierto && (
        <ModalGestionEtiquetas onClose={() => setModalEtiquetasAbierto(false)} />
      )}

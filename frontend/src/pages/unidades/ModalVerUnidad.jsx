const ModalVerUnidad = ({ unidad, cerrarModal }) => {
    ...
    return (
        <div style={S.overlay} onClick={(e) => e.target === e.currentTarget && cerrarModal()}>
            <div style={S.modal}>
                <div style={S.header}>
                    <h3 style={S.titulo}>Detalle de Unidad</h3>
                    <button style={S.btnX} onClick={cerrarModal}>✕</button>
                </div>
                <div style={S.body}>
                    <div style={S.fila}>
                        <span style={S.etiqueta}>ID</span>
                        <span style={S.valor}>{unidad.id}</span>
                    </div>
                    <div style={S.fila}>
                        <span style={S.etiqueta}>Nombre</span>
                        <span style={S.valor}>{unidad.nombre}</span>
                    </div>
                    <div style={S.fila}>
                        <span style={S.etiqueta}>Productos relacionados</span>
                        <span style={S.valor}>{unidad.cantidadProductosRelacionados ?? 0} producto(s)</span>
                    </div>
                    <div style={S.fila}>
                        <span style={S.etiqueta}>Estado</span>
                        <span style={S.valor}>{badgeEstado(unidad.estado)}</span>
                    </div>
                </div>
                <div style={S.footer}>

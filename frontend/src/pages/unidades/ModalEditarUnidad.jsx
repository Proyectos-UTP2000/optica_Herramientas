...
const ModalEditarUnidad = ({ unidad, cerrarModal, recargarTabla }) => {
    ...
    useEffect(() => { if (unidad) setNombre(unidad.nombre || ""); }, [unidad]);

    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!nombre.trim()) {
            mostrarAlerta("Campo requerido", "El nombre de la unidad es obligatorio.", "warning");
            return;
        }
        setLoading(true);
        try {
            await axios.put(`/api/v1/unidades/${unidad.id}`, { nombre: nombre.trim() }, { headers });
            Toast.fire({ icon: "success", title: "Unidad actualizada correctamente" });
            recargarTabla();
            cerrarModal();
        } catch (e) {
    ...
    return (
        <div style={S.overlay} onClick={(e) => e.target === e.currentTarget && cerrarModal()}>
            <div style={S.modal}>
                <div style={S.header}>
                    <h3 style={S.titulo}>Editar Unidad</h3>
                    <button style={S.btnX} onClick={cerrarModal}>✕</button>
                </div>
                <form onSubmit={handleSubmit}>
                    <div style={S.body}>
                        <label className="label-control">Nombre *</label>
                        <input className="input-control" type="text" value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Ej: Caja, Unidad, Litro..." autoFocus />
                    </div>
                    <div style={S.footer}>

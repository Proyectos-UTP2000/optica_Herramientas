...
const ModalHistorialInventario = ({ producto, cerrarModal }) => {
    ...
    const cargarHistorial = async () => {
      ...
      try {
        const response = await axios.get(
          `/api/v1/inventario/productos/${idProducto}/movimientos`,
          {
  ...
  const nombreProducto =
    producto?.productoNombre || producto?.nombre || `Artículo #${idProducto}`;
  ...
  const cambiarFechaDesde = (valor) => {
    if (valor > hoyString) {
      Toast.fire({
        icon: "warning",
        title: "La fecha inicial no puede ser mayor a hoy",
      });
    ...
    if (fechaHastaInput && valor > fechaHastaInput) {
      Toast.fire({
        icon: "warning",
        title: "La fecha inicial no puede superar a la fecha final",
      });
  ...
  const cambiarFechaHasta = (valor) => {
    if (valor > hoyString) {
      Toast.fire({
        icon: "warning",
        title: "La fecha final no puede ser mayor a hoy",
      });
  ...
  const movimientosFiltrados = movimientos.filter((mov) => {
    ...
    if (mov.fecha) {
      ...
      if (fechaDesdeAplicada && fechaMovimiento < fechaDesdeAplicada)
        return false;
      if (fechaHastaAplicada && fechaMovimiento > fechaHastaAplicada)
        return false;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {producto?.productoCodigo && (
          <span style={{ fontSize: "13px", color: "var(--text-muted)" }}>
            SKU: <code>{producto.productoCodigo}</code> |{" "}
          </span>
        )}
        Stock actual en tienda:{" "}
        <strong>{producto?.stockActual ?? 0} uds</strong>
      </div>
      ...
      <SeccionLabel
        text={`Línea de Tiempo de Auditoría (${movimientosFiltrados.length})`}
      />
      <div
        style={{ maxHeight: "280px", overflowY: "auto", paddingRight: "5px" }}
      >
        {cargando ? (
          <div
            style={{
          ...
          >
            No se encontraron variaciones que coincidan con los filtros
            aplicados.
          </div>
        ) : (
          <div
                ...
                >
                  <div>
                    ...
                    >
                      <Layers size={12} />
                      <span>
                        Stock: {mov.stockPrevio} uds. →{" "}
                        <strong>{mov.stockNuevo} uds.</strong>
                      </span>
                    </div>
                    ...
                    {mov.empleadoNombre && (
                      ...
                      >
                        <Person size={12} />
                        <span>
                          Realizado por: <span>{mov.empleadoNombre}</span>
                        </span>
                      </div>
                    )}

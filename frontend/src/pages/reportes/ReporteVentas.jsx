...
const ReporteVentas = () => {
  ...
  const cargarReporte = useCallback(async () => {
    ...
    try {
      ...
      Toast.fire({
        icon: "error",
        title: mensajeBackend(error, "No se pudo cargar el reporte de ventas"),
      });
    } finally {
      setCargando(false);
    }
    ...
    const timer = window.setTimeout(() => {
      Promise.all([listarProductosReporte(), listarEmpleadosReporte()])
        ...
        .catch(() =>
          Toast.fire({
            icon: "error",
            title: "No se pudieron cargar filtros de ventas",
          }),
        );
      cargarReporte();
    }, 0);
  ...
  return (
    <div className="page-container">
      ...
      <div className="report-filter-grid">
        ...
        <div>
          <label className="form-label">Cliente/comprobante</label>
          <input
            className="input-control"
            value={texto}
            onChange={(e) => setTexto(e.target.value)}
            placeholder="Cliente, comprobante..."
          />
        </div>
        ...
        <div>
          <label className="form-label">Nro. venta</label>
          <input
            className="input-control"
            value={numeroVenta}
            onChange={(e) => setNumeroVenta(e.target.value)}
            placeholder="# o comprobante"
          />
        </div>
        <div>
      ...
      <div className="report-summary-row">
        <div className="stat-card">
          <span>{reporte?.cantidadVentas || 0} venta(s)</span>
        </div>
        <div className="stat-card">
          <strong>{formatoMoneda(reporte?.totalVentas)}</strong>
            ...
            {ventas.length === 0 ? (
              ...
              ventas.map((venta) => (
                <tr key={venta.ventaId}>
                  <td>{formatoFechaHora(venta.fecha)}</td>
                  <td>{venta.clienteNombre || "--"}</td>
                  <td>{venta.empleadoNombre || "--"}</td>
                  <td>{venta.numeroComprobante || "--"}</td>
                  <td title={venta.productosResumen}>
      ...
      {detalle && (
        <div className="modal-overlay-light" onClick={() => setDetalle(null)}>
          <div className="detail-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p>
              <strong>Cliente:</strong> {detalle.clienteNombre} |{" "}

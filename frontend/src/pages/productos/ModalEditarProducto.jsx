...
const ModalEditarProducto = ({ producto, cerrarModal, recargarTabla }) => {
  ...
  const [visibleWeb, setVisibleWeb] = useState(producto?.visibleWeb ?? true);
  ...
  const handleGuardar = async () => {
    ...
    const productoData = {
      ...
      descripcionWeb: producto?.descripcionWeb || null,
      idEtiquetas: producto?.etiquetas
        ? producto.etiquetas.map((e) => e.id)
        : [],
    ...
    >
      ...
      {idUnidadCompra && idUnidadVenta && parseInt(factorConversion) > 0 && (
        ...
        >
          ...
          <span>
            Cada 1 "{textoUnidadCompra}" ingresada modificará el inventario por{" "}
            {factorConversion} "{textoUnidadVenta}".
          </span>
        </div>
      )}

...
const ModalCrearProducto = ({ cerrarModal, recargarTabla }) => {
  ...
  const validarFormulario = () => {
    ...
    if (parseInt(stock) < 0) err.stock = "El stock inicial no puede ser negativo";
    ...
    >
      ...
      <div className="form-grid">
        <div style={{ gridColumn: "span 2" }}>
          ...
          <input className={inputError("nombre")} value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Ej. Lente Antireflex Premium" />
          {msgError("nombre")}
        </div>
        <div>
          <label className="label-control">Código SKU / Barras</label>
          <input className="input-control" value={codigo} onChange={(e) => setCodigo(e.target.value)} placeholder="Opcional" />
        </div>
        <div>
          <label className="label-control">Modelo / Serie</label>
          <input className="input-control" value={modelo} onChange={(e) => setModelo(e.target.value)} placeholder="Ej. Aviator Classic" />
        </div>
      </div>

      <div style={{ marginTop: "10px" }}>
        <label className="label-control">Descripción Detallada</label>
        <textarea className={inputError("descripcion")} value={descripcion} onChange={(e) => setDescripcion(e.target.value)} placeholder="Especificaciones técnicas del producto..." rows="2" style={{ width: "100%", padding: "8px", borderRadius: "6px", border: "1px solid #cbd5e1" }} />
      ...
      <div className="form-grid">
        <div>
          ...
          <input type="number" className={inputError("costo")} value={costo} onChange={(e) => setCosto(e.target.value)} placeholder="0.00" />
          {msgError("costo")}
        </div>
        <div>
          <label className="label-control">Precio Venta Público</label>
          <input type="number" className={inputError("precio")} value={precio} onChange={(e) => setPrecio(e.target.value)} placeholder="0.00" />
          {msgError("precio")}
        </div>
        <div>
      ...
      {idUnidadCompra && idUnidadVenta && parseInt(factorConversion) > 0 && (
        <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", padding: "10px", borderRadius: "6px", color: "#1e40af", fontSize: "12px", marginTop: "12px", display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <span>Cada 1 "{textoUnidadCompra}" ingresada registrará automáticamente {factorConversion} "{textoUnidadVenta}" en el almacén.</span>
        </div>
      )}

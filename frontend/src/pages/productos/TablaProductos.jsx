...
const TablaProductos = ({ productos, cargando, recargarTabla, onEditarProducto, onVerProducto }) => {
  ...
  const handleEliminar = async (id, nombre) => {
    ...
    if (confirmacion.isConfirmed) {
      ...
      try {
        ...
        Toast.fire({
          icon: "error",
          title: error.response?.data?.message || "No se pudo eliminar el producto"
        });
      }
    }
  };
  ...
  const productosFiltrados = productos.filter((p) => {
    ...
    const matchesCategoria = !filtroCategoria || String(p.idCategoria) === filtroCategoria;
    ...
    return matchesSearch && matchesCategoria && matchesMarca && matchesTipo && matchesStock && matchesEstado && matchesPrecioMin && matchesPrecioMax;
  });
      ...
      {showFiltros && (
        <div style={{
          ...
          <div>
            ...
            <select className="input-control" value={filtroCategoria} onChange={(e) => setFiltroCategoria(e.target.value)}>
          ...
          <div>
            ...
            <select className="input-control" value={filtroMarca} onChange={(e) => setFiltroMarca(e.target.value)}>
              <option value="">Todas</option>
          ...
          <div>
            ...
            <select className="input-control" value={filtroTipo} onChange={(e) => setFiltroTipo(e.target.value)}>
          ...
          <div>
            ...
            <select className="input-control" value={filtroStock} onChange={(e) => setFiltroStock(e.target.value)}>
              <option value="">Todos</option>
          ...
          <div>
            ...
            <select className="input-control" value={filtroEstado} onChange={(e) => setFiltroEstado(e.target.value)}>
              <option value="">Todos</option>
          ...
          <div>
            ...
            <input type="number" className="input-control" value={precioMin} onChange={(e) => setPrecioMin(e.target.value)} placeholder="0.00" />
          </div>
          <div>
            <label className="label-control">Precio Máx (S/.)</label>
            <input type="number" className="input-control" value={precioMax} onChange={(e) => setPrecioMax(e.target.value)} placeholder="999" />
          </div>
                ...
                <tr key={p.id} style={{ borderBottom: "1px solid #f1f5f9", color: "#334155" }}>
                  ...
                  <td style={{ padding: "12px 8px" }}>
                    <span style={{ fontSize: "12px", background: "#f1f5f9", padding: "2px 6px", borderRadius: "4px" }}>
                      1 {p.unidadCompraNombre || "Caja"} = {p.factorConversion} {p.unidadVentaNombre || "Und"}
                    </span>
                  </td>

...
const CotizacionesAdmin = () => {
  ...
  const cargarCotizaciones = async () => {
    try {
      ...
      Toast.fire({
        icon: "error",
        title: "No se pudieron obtener las cotizaciones",
      });
    } finally {
      setCargando(false);
    }
  ...
  const handleCambiarEstado = async (id, nuevoEstado) => {
    ...
    try {
      ...
      Toast.fire({
        icon: "success",
        title: `Cotización marcada como ${nuevoEstado}`,
      });
  ...
  const handleContactarWhatsapp = (coti) => {
    ...
    if (!telefono) {
      Toast.fire({
        icon: "warning",
        title: "El cliente no registró un número telefónico válido",
      });
      return;
    }

    const itemsStr = coti.detalles
      .map((d) => `- ${d.cantidad}x ${d.productoNombre}`)
      .join("%0A");
  ...
  const cotizacionesFiltradas = cotizaciones.filter((c) => {
    ...
    const coincideBusqueda =
      cotiCliente.includes(query) ||
      cotiDoc.includes(query) ||
      cotiTel.includes(query);
                ...
                <tr key={coti.id}>
                  ...
                  <td style={styles.td}>
                    ...
                    <div style={{ fontSize: "11px", color: "#94a3b8" }}>
                      {new Date(coti.fechaCreacion).toLocaleTimeString(
                        "es-PE",
                  ...
                  <td style={styles.td}>
                    ...
                    <div style={{ fontSize: "12px", color: "#64748b" }}>
                      {coti.clienteDocumento || "Sin Documento"}
                    </div>
                  </td>
                  <td style={styles.td}>
          ...
          >
            <div style={styles.modalHeader}>
              <h3 style={styles.modalTitle}>
                Detalle de Cotización #{seleccionada.id}
              </h3>
              <button
            ...
            <div style={styles.modalBody}>
              ...
              <div style={styles.gridInfo}>
                <div style={styles.infoItem}>
                  ...
                  <p style={styles.infoValue}>
                    <Person /> {seleccionada.clienteNombre}
                  </p>
                </div>
                <div style={styles.infoItem}>
                  <div style={styles.infoLabel}>Documento (DNI/RUC)</div>
                  <p style={styles.infoValue}>
                    {seleccionada.clienteDocumento || "No Registrado"}
                  </p>
                </div>
                <div style={styles.infoItem}>
                  ...
                  <p style={styles.infoValue}>
                    ...
                    {seleccionada.clienteTelefono || "No Registrado"}
                  </p>
                </div>
                <div style={styles.infoItem}>
                  ...
                  <p style={styles.infoValue}>
                    <CalendarDate />{" "}
                    {new Date(seleccionada.fechaCreacion).toLocaleString(
                      "es-PE",
                    )}
                  </p>
                </div>
                    ...
                    <tbody>
                      {seleccionada.detalles.map((det) => (
                        <tr key={det.id}>
                          ...
                          <td style={{ ...styles.tdDetail, fontWeight: "600" }}>
                            {det.productoNombre}
                          </td>
                          <td style={styles.tdDetail}>{det.cantidad}</td>
                          <td style={styles.tdDetail}>
                            {formatoMoneda(det.precioLista)}
                          </td>
                          <td
                            style={{
                              ...styles.tdDetail,
                      ...
                      <tr style={{ backgroundColor: "#f8fafc" }}>
                        ...
                        >
                          {formatoMoneda(seleccionada.totalEstimado)}
                        </td>
                      </tr>
                    </tbody>
                  </table>
              ...
              {seleccionada.observaciones && (
                <div style={styles.infoItem}>
                  <div style={styles.infoLabel}>
                    <ChatLeftTextFill size={10} /> Notas / Observaciones del
                    Cliente
                  </div>
                  <p
                    style={{
              ...
              >
                ...
                <span style={styles.badge(seleccionada.estado)}>
                  {seleccionada.estado}
                </span>
              </div>
            </div>
              ...
              <div style={styles.btnGroup}>
                ...
                {seleccionada.estado === "PENDIENTE" && (
                  <button
                    ...
                    onClick={() =>
                      handleCambiarEstado(seleccionada.id, "CONTACTADO")
                    }
                ...
                {(seleccionada.estado === "PENDIENTE" ||
                  ...
                  <>
                    <button
                      ...
                      onClick={() =>
                        handleCambiarEstado(seleccionada.id, "PROCESADO")
                      }
                      disabled={guardandoEstado}
                    >
                      Procesar
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleCambiarEstado(seleccionada.id, "ANULADO")
                      }
                      disabled={guardandoEstado}
                    >
                      Anular

...
const ModalCrearCategoria = ({ cerrarModal, recargarTabla }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.post("/api/v1/categorias", { nombre: nombre.trim() }, { headers });
            Toast.fire({ icon: "success", title: "Categoría creada correctamente" });
            recargarTabla();
            cerrarModal();
        } catch (e) {
            mostrarAlerta("Error al crear", e.response?.data?.message || "No se pudo crear la categoría.", "error");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div style={S.overlay} onClick={(e) => e.target === e.currentTarget && cerrarModal()}>
            <div style={S.modal}>
                <div style={S.header}>
                    <h3 style={S.titulo}>Nueva Categoría</h3>

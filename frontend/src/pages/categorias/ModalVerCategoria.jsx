const ModalVerCategoria = ({ categoria, cerrarModal }) => {
    ...
    return (
        <div style={S.overlay} onClick={(e) => e.target === e.currentTarget && cerrarModal()}>
            <div style={S.modal}>
                <div style={S.header}>
                    <h3 style={S.titulo}>Detalle de Categoría</h3>
                    <button style={S.btnX} onClick={cerrarModal}>✕</button>
                </div>
                <div style={S.body}>
                    <div style={S.fila}>
                        <span style={S.etiqueta}>ID</span>
                        <span style={S.valor}>{categoria.id}</span>
                    </div>
                    <div style={S.fila}>
                        <span style={S.etiqueta}>Nombre</span>
                        <span style={S.valor}>{categoria.nombre}</span>
                    </div>
                    <div style={S.fila}>
                        <span style={S.etiqueta}>Productos relacionados</span>
                        <span style={S.valor}>{categoria.cantidadProductosRelacionados ?? 0} producto(s)</span>
                    </div>
                    <div style={S.fila}>
                        <span style={S.etiqueta}>Estado</span>
                        <span style={S.valor}>{badgeEstado(categoria.estado)}</span>
                    </div>
                </div>
                <div style={S.footer}>
...
const S = {
    ...
    fila: { display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: "12px", borderBottom: "1px solid #f1f5f9" },
    etiqueta: { fontSize: "13px", color: "#64748b", fontWeight: "500" },
    valor: { fontSize: "13.5px", color: "#0f172a", fontWeight: "600" },
    footer: { display: "flex", justifyContent: "flex-end", padding: "14px 20px", borderTop: "1px solid #e2e8f0", background: "#f8fafc" },
};

export default ModalVerCategoria;